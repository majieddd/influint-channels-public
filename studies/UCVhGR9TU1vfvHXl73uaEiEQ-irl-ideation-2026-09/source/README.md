...
The five solo formats and 25 episode briefs are authored in `../solo-formats.json`. `solo_sections.py` renders their Ideation, Creative, Production and Data Analysis additions. Solo IDs 101-125 include direct source-video links, the original source-video thumbnails, relevance notes, and composition prompts. Original gallery IDs 1-100 keep the existing pagination and thumbnail mappings.

The selected JPEGs, WebP previews and exact generation prompts are listed in `../thumbnail-manifest.json`. The full download contains all 100 JPEGs and prompts. The 25 additional Niz thumbnail concepts are listed in `../solo-thumbnail-manifest.json` with source-video links, source thumbnails, prompt checksums, generated image checksums, and responsive WebP previews. The available built-in image generator was used; its model identity is unverified and was not confirmed as GPT Image 2.5 Flare.
